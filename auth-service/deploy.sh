...
# NUCLEAR OPTION: Find and remove ANY container with "notification-auth-service" in the name
echo "🧹 Cleaning up any existing containers..."
# List ALL containers and check their names directly (handle leading slash)
docker ps -a --format "{{.ID}} {{.Names}}" | grep -iE "(notification-auth-service|/notification-auth-service)" | awk '{print $1}' | while read container_id; do
...
# Also try exact name (with and without leading slash)
...
docker ps -a --format "{{.ID}} {{.Names}}" | awk '$2 ~ /notification-auth-service/ {print $1}' | while read container_id; do
  if [ -n "$container_id" ]; then
...
CONFLICT_ID=$(docker ps -a --format "{{.ID}} {{.Names}}" | awk '$2 ~ /notification-auth-service/ {print $1}' | head -1)
...
if echo "$ALL_CONTAINERS" | grep -qi "notification-auth-service"; then
  echo "⚠️  Found container right before start, removing..."
  echo "$ALL_CONTAINERS" | grep -i "notification-auth-service" | awk '{print $1}' | while read id; do
