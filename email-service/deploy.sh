...
docker ps -a --filter "name=notification-email-service" -q | xargs -r docker rm -f 2>/dev/null || true
# Remove any container with email-service in the name
docker ps -a --filter "name=email-service" -q | xargs -r docker rm -f 2>/dev/null || true
