...
for container_id in $(docker ps -a --filter "name=notification-user-service" --format "{{.ID}}" 2>/dev/null); do
  docker rm -f "$container_id" 2>/dev/null || true
done
# Method 3: Find and remove by service name pattern
for container_id in $(docker ps -a --filter "name=user-service" --format "{{.ID}}" 2>/dev/null); do
  docker rm -f "$container_id" 2>/dev/null || true
done
...
for container_id in $(docker ps -aq 2>/dev/null); do
  ...
  if [[ "$container_name" == *"notification-user-service"* ]] || [[ "$container_name" == *"/notification-user-service"* ]]; then
    docker rm -f "$container_id" 2>/dev/null || true
  fi
done
