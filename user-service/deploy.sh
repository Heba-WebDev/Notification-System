...
docker ps -a --format "{{.ID}} {{.Names}}" | grep -i "notification-user-service" | awk '{print $1}' | while read container_id; do
...
# Also try exact name (handles case where grep doesn't catch it)
