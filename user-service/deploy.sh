...
# NUCLEAR OPTION: Find and remove ANY container with "notification-user-service" in the name
echo "🧹 Cleaning up any existing containers..."
# List ALL containers and check their names directly (handle leading slash)
docker ps -a --format "{{.ID}} {{.Names}}" | grep -iE "(notification-user-service|/notification-user-service)" | awk '{print $1}' | while read container_id; do
...
docker ps -a --format "{{.ID}} {{.Names}}" | awk '$2 ~ /notification-user-service/ {print $1}' | while read container_id; do
  if [ -n "$container_id" ]; then
...
CONFLICT_ID=$(docker ps -a --format "{{.ID}} {{.Names}}" | awk '$2 ~ /notification-user-service/ {print $1}' | head -1)
...
if echo "$ALL_CONTAINERS" | grep -qi "notification-user-service"; then
  echo "⚠️  Found container right before start, removing..."
  echo "$ALL_CONTAINERS" | grep -i "notification-user-service" | awk '{print $1}' | while read id; do
