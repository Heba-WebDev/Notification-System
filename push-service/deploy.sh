...
for container_id in $(docker ps -a --filter "name=notification-push-service" --format "{{.ID}}" 2>/dev/null); do
  docker rm -f "$container_id" 2>/dev/null || true
done
# Method 3: Find and remove by service name pattern
for container_id in $(docker ps -a --filter "name=push-service" --format "{{.ID}}" 2>/dev/null); do
  docker rm -f "$container_id" 2>/dev/null || true
done
...
for container_id in $(docker ps -aq 2>/dev/null); do
  ...
  if [[ "$container_name" == *"notification-push-service"* ]] || [[ "$container_name" == *"/notification-push-service"* ]]; then
    docker rm -f "$container_id" 2>/dev/null || true
  fi
done
