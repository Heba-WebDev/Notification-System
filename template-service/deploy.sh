...
if echo "$ALL_CONTAINERS" | grep -qi "notification-template-service"; then
  echo "⚠️  Found container right before start, removing..."
  echo "$ALL_CONTAINERS" | grep -i "notification-template-service" | awk '{print $1}' | while read id; do
